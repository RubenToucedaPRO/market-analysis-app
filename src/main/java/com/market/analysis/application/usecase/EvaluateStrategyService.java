...
public class EvaluateStrategyService implements EvaluateStrategyUseCase {
    ...
    @Override
    public AnalysisResult evaluateStrategy(Strategy strategy, Stock tickerData) {
        if (strategy == null) {
            throw new IllegalArgumentException("Strategy cannot be null");
        }
        if (tickerData == null) {
            throw new IllegalArgumentException("Ticker data cannot be null");
        }

        log.info("Evaluating strategy '{}' for ticker '{}'", strategy.getName(), tickerData.getTicker());
        ...
        for (Rule rule : strategy.getRules()) {
            RuleResult result = ruleEvaluator.evaluate(rule, tickerData);
        ...
        // Generate summary
        String summary = generateSummary(strategy, tickerData.getTicker(), ruleResults, overallPassed);
